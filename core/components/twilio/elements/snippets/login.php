...
if (!($twilio instanceof Twilio) || !$twilio->init()) {
    $modx->log(modX::LOG_LEVEL_ERROR, '[twilio.login] could not load the required class on line: ' . __LINE__);
    return;
}
...
if ($modx->user->hasSessionContext($modx->context->key) && !$reVerify) {
    if ($loginResourceUrl) {
        ...
        return $twilio->getChunk($alreadyLoggedInTpl);
    }
}

$loggedIn = $twilio->login($loginContexts, true, $reVerify);
if ($loggedIn === true) {
    if ($loginResourceUrl) {
        $modx->sendRedirect($loginResourceUrl);
        return;
    } else {
        return $twilio->getChunk($successfulLoginTpl);
    }
}

if ($logoutOnFailedVerification) $twilio->logout($loginContexts);

$userState = $twilio->getUserState();
if ($userState === Twilio::STATE_VERIFIED) {
    return $twilio->getChunk($failedLoginTpl);
}

return $twilio->getChunk($TPLs[$userState . 'Tpl'], $twilio->getUser(false, false));
